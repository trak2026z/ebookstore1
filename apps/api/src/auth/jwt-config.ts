...
export function parseJwtConfig(environment: NodeJS.ProcessEnv): JwtConfig {
  return {
    secret: parseJwtSecret(environment["JWT_SECRET"]),
    accessTokenTtlSeconds: parseAccessTokenTtl(environment["JWT_ACCESS_TOKEN_TTL_SECONDS"]),
  };
}

function parseJwtSecret(value: string | undefined): string {
  if (value === undefined || value.length < MINIMUM_JWT_SECRET_LENGTH) {
    throw new Error(`JWT_SECRET must contain at least ${MINIMUM_JWT_SECRET_LENGTH} characters.`);
  }

  return value;
}

function parseAccessTokenTtl(value: string | undefined): number {
  if (value === undefined) {
