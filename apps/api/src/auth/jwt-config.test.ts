...
const validSecret = "test-secret-with-at-least-thirty-two-characters";
...
describe("parseJwtConfig", () => {
  ...
  it.each([undefined, "", "too-short"])("rejects an invalid JWT secret %j", (jwtSecret) => {
    expect(() =>
      parseJwtConfig({
  ...
  it.each(["", "abc", "1.5", "0", "-1"])("rejects an invalid TTL %j", (ttl) => {
    expect(() =>
      parseJwtConfig({
        JWT_SECRET: validSecret,
